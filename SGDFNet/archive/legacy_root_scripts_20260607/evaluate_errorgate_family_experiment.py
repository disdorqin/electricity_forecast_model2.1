...
def main() -> None:
    ...
    if state.get("current_stage") != "ERRORGATE_FAMILY_EXPERIMENT_RAN" or state.get("current_branch") != "errorgate_family_experiment_run_complete":
        raise ValueError("Error-gate family evaluation is only allowed from ERRORGATE_FAMILY_EXPERIMENT_RAN.")

    instance_dir = Path(state["active_errorgate_instance_dir"])
    ...
    if (
        ...
        or (seg916_delta <= 0.10 and seg916_capped_delta <= -0.15 and top10_delta <= -0.40)
    ):
        decision = "KEEP"
        next_action = "expand this error-gate family with the next candidate or package it as the current best new family"
    else:
        decision = "REJECT"
        next_action = "try the next error-gate candidate if one remains, otherwise close this family and move on"
    ...
    (instance_dir / "decision.md").write_text(
        "# Error-Gate Family Experiment Decision\n\n"
        f"- Decision: `{decision}`\n"
        f"- Reason: `{reason}`\n"
    ...
    summary_md = ERRORGATE_DIR / "errorgate_family_experiment_summary.md"
    summary_md.write_text(
        "# Error-Gate Family Experiment Summary\n\n"
    ...
    state["current_stage"] = "ERRORGATE_FAMILY_EXPERIMENT_EVALUATED"
    state["current_branch"] = "errorgate_family_experiment_decision_complete"
    state["last_errorgate_decision"] = decision
    state["allowed_next_actions"] = [
        "review the error-gate family decision",
        "promote the error-gate family if it is KEEP",
    ...
    AUTO_SUMMARY_PATH.write_text(
        ...
        "- Active stage: `ERRORGATE_FAMILY_EXPERIMENT_EVALUATED`\n"
        "- Current branch state: `errorgate_family_experiment_decision_complete`\n"
        f"- Active error-gate family experiment: `{manifest['experiment_id']}`\n"
        f"- Decision: `{decision}`\n",
        encoding="utf-8",
    )
