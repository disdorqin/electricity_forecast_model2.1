...
def main() -> None:
    ...
    if state.get("current_stage") != "KICKOFF_SELECTIVE_GATE_EXPERIMENT_RAN" or state.get("current_branch") != "kickoff_selective_gate_experiment_run_complete":
        raise ValueError("Kickoff selective-gate evaluation is only allowed from KICKOFF_SELECTIVE_GATE_EXPERIMENT_RAN.")

    instance_dir = Path(state["active_kickoff_selective_gate_instance_dir"])
    ...
    keep = (
        ...
        and raw_overall_delta <= 0.15
        and raw_seg_delta <= 0.25
    )
    ...
    next_action = (
        "try the next kickoff selective-gate candidate if one remains, otherwise close this family and decide whether to package the evidence"
        if decision == "REJECT"
        else "package this kickoff selective-gate candidate as the current best kickoff continuation"
    )
    ...
    (instance_dir / "decision.md").write_text(
        "# Kickoff Selective Gate Experiment Decision\n\n"
        f"- Decision: `{decision}`\n"
        f"- Reason: `{reason}`\n"
    ...
    summary_md = KSGATE_DIR / "kickoff_selective_gate_experiment_summary.md"
    summary_md.write_text(
        "# Kickoff Selective Gate Experiment Summary\n\n"
    ...
    state["current_stage"] = "KICKOFF_SELECTIVE_GATE_EXPERIMENT_EVALUATED"
    state["current_branch"] = "kickoff_selective_gate_experiment_decision_complete"
    state["last_kickoff_selective_gate_decision"] = decision
    state["allowed_next_actions"] = [
        "review the kickoff selective-gate decision",
    ...
    AUTO_SUMMARY_PATH.write_text(
        ...
        "- Active stage: `KICKOFF_SELECTIVE_GATE_EXPERIMENT_EVALUATED`\n"
        "- Current branch state: `kickoff_selective_gate_experiment_decision_complete`\n"
        f"- Active kickoff selective-gate experiment: `{manifest['experiment_id']}`\n"
        f"- Decision: `{decision}`\n",
        encoding="utf-8",
    )
