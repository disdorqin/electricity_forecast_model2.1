...
def main() -> None:
    ...
    if state.get("current_stage") != "KICKOFF_TAILCAL_EXPERIMENT_RAN" or state.get("current_branch") != "kickoff_tailcal_experiment_run_complete":
        raise ValueError("Kickoff tail-calibration evaluation is only allowed from KICKOFF_TAILCAL_EXPERIMENT_RAN.")

    instance_dir = Path(state["active_kickoff_tailcal_instance_dir"])
    ...
    keep = (
        overall_gain >= 0.03
        and (seg_gain >= 0.08 or tail_gain >= 0.30)
        and raw_overall_delta <= 0.20
        and raw_seg_delta <= 0.40
    )
    ...
    next_action = (
        "try the next kickoff tail-calibration candidate if one remains, otherwise close this family and decide whether to package the near-miss evidence"
        if decision == "REJECT"
        else "package this kickoff tail-calibration candidate as the current best kickoff follow-up continuation"
    )
    ...
    (instance_dir / "decision.md").write_text(
        "# Kickoff Tail Calibration Experiment Decision\n\n"
        f"- Decision: `{decision}`\n"
        f"- Reason: `{reason}`\n"
    ...
    summary_md = KTCAL_DIR / "kickoff_tailcal_experiment_summary.md"
    summary_md.write_text(
        "# Kickoff Tail Calibration Experiment Summary\n\n"
    ...
    state["current_stage"] = "KICKOFF_TAILCAL_EXPERIMENT_EVALUATED"
    state["current_branch"] = "kickoff_tailcal_experiment_decision_complete"
    state["last_kickoff_tailcal_decision"] = decision
    state["allowed_next_actions"] = [
        "review the kickoff tail-calibration decision",
    ...
    AUTO_SUMMARY_PATH.write_text(
        ...
        "- Active stage: `KICKOFF_TAILCAL_EXPERIMENT_EVALUATED`\n"
        "- Current branch state: `kickoff_tailcal_experiment_decision_complete`\n"
        f"- Active kickoff tail-calibration experiment: `{manifest['experiment_id']}`\n"
        f"- Decision: `{decision}`\n",
        encoding="utf-8",
    )
