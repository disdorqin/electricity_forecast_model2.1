...
def main() -> None:
    ...
    if state.get("current_stage") != "MONTHHOUR_FAMILY_EXPERIMENT_RAN" or state.get("current_branch") != "monthhour_family_experiment_run_complete":
        raise ValueError("Month-hour family evaluation is only allowed from MONTHHOUR_FAMILY_EXPERIMENT_RAN.")

    instance_dir = Path(state["active_monthhour_instance_dir"])
    ...
    if (
        ...
        or (seg916_delta <= 0.10 and seg916_capped_delta <= -0.15 and top10_delta <= -0.30)
    ):
        decision = "KEEP"
        next_action = "expand this month-hour family with the next candidate or package it as the current best new family"
    else:
        decision = "REJECT"
        next_action = "try the next month-hour candidate if one remains, otherwise close this family and move on"
    ...
    (instance_dir / "decision.md").write_text(
        "# Month-Hour Family Experiment Decision\n\n"
        f"- Decision: `{decision}`\n"
        f"- Reason: `{reason}`\n"
    ...
    summary_md = MONTHHOUR_DIR / "monthhour_family_experiment_summary.md"
    summary_md.write_text(
        "# Month-Hour Family Experiment Summary\n\n"
    ...
    state["current_stage"] = "MONTHHOUR_FAMILY_EXPERIMENT_EVALUATED"
    state["current_branch"] = "monthhour_family_experiment_decision_complete"
    state["last_monthhour_decision"] = decision
    state["allowed_next_actions"] = [
        "review the month-hour family decision",
        "promote the month-hour family if it is KEEP",
    ...
    AUTO_SUMMARY_PATH.write_text(
        ...
        "- Active stage: `MONTHHOUR_FAMILY_EXPERIMENT_EVALUATED`\n"
        "- Current branch state: `monthhour_family_experiment_decision_complete`\n"
        f"- Active month-hour family experiment: `{manifest['experiment_id']}`\n"
        f"- Decision: `{decision}`\n",
        encoding="utf-8",
    )
