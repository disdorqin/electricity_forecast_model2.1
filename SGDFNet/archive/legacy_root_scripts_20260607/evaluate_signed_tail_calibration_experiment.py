...
def main() -> None:
    ...
    if state.get("current_stage") != "SIGNED_TAIL_CALIBRATION_EXPERIMENT_RAN" or state.get("current_branch") != "signed_tail_calibration_experiment_run_complete":
        raise ValueError("Signed-tail calibration family evaluation is only allowed from SIGNED_TAIL_CALIBRATION_EXPERIMENT_RAN.")

    instance_dir = Path(state["active_signed_tail_calibration_instance_dir"])
    ...
    next_action = (
        "try the next signed-tail calibration candidate if one remains, otherwise close this family and decide whether to promote the best near-miss"
        if decision == "REJECT"
        else "package this signed-tail calibration candidate as the current best automation-side point-repair branch"
    )
    ...
    (instance_dir / "decision.md").write_text(
        "# Signed-Tail Calibration Experiment Decision\n\n"
        f"- Decision: `{decision}`\n"
        f"- Reason: `{reason}`\n"
    ...
    summary_md = CAL_DIR / "signed_tail_calibration_experiment_summary.md"
    summary_md.write_text(
        "# Signed-Tail Calibration Experiment Summary\n\n"
    ...
    state["current_stage"] = "SIGNED_TAIL_CALIBRATION_EXPERIMENT_EVALUATED"
    state["current_branch"] = "signed_tail_calibration_experiment_decision_complete"
    state["last_signed_tail_calibration_decision"] = decision
    state["allowed_next_actions"] = [
        "review the signed-tail calibration decision",
    ...
    AUTO_SUMMARY_PATH.write_text(
        ...
        "- Active stage: `SIGNED_TAIL_CALIBRATION_EXPERIMENT_EVALUATED`\n"
        "- Current branch state: `signed_tail_calibration_experiment_decision_complete`\n"
        f"- Active signed-tail calibration experiment: `{manifest['experiment_id']}`\n"
        f"- Decision: `{decision}`\n",
        encoding="utf-8",
    )
