...
def main() -> None:
    ...
    if state.get("current_stage") != "STRUCTURAL_FAMILY_EXPERIMENT_RAN" or state.get("current_branch") != "structural_family_experiment_run_complete":
        raise ValueError("Structural family evaluation is only allowed from STRUCTURAL_FAMILY_EXPERIMENT_RAN.")
    ...
    if (
        ...
        or (seg916_delta <= 0.10 and capped_delta <= 0.10 and top10_delta <= -0.30)
    ):
        decision = "KEEP"
        next_action = "expand this structural family with the next candidate or package it as the current best new main-model family"
    else:
        decision = "REJECT"
        next_action = "try the next structural candidate if one remains, otherwise close this family and move on"
    ...
    (instance_dir / "decision.md").write_text(
        "# Structural Family Experiment Decision\n\n"
        f"- Decision: `{decision}`\n"
        f"- Reason: `{reason}`\n"
    ...
    summary_md = STRUCTURAL_DIR / "structural_family_experiment_summary.md"
    summary_md.write_text(
        "# Structural Family Experiment Summary\n\n"
    ...
    state["current_stage"] = "STRUCTURAL_FAMILY_EXPERIMENT_EVALUATED"
    state["current_branch"] = "structural_family_experiment_decision_complete"
    state["last_structural_decision"] = decision
    state["allowed_next_actions"] = [
        "review the structural family decision",
        "promote the structural family if it is KEEP",
    ...
    AUTO_SUMMARY_PATH.write_text(
        ...
        "- Active stage: `STRUCTURAL_FAMILY_EXPERIMENT_EVALUATED`\n"
        "- Current branch state: `structural_family_experiment_decision_complete`\n"
