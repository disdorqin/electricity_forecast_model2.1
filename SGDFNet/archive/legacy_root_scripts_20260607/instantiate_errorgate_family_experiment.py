...
from errorgate_family_cycle_catalog import list_errorgate_family_candidates
...
INSTANCES_DIR = ERRORGATE_DIR / "instances"
RESULTS_CSV = ERRORGATE_DIR / "errorgate_family_experiment_results.csv"
...
def _select_next_candidate() -> dict | None:
    ...
    for candidate in list_errorgate_family_candidates():
...
def main() -> None:
    ...
    allowed_states = {
        ("ERRORGATE_FAMILY_PREPARED", "errorgate_family_manifest_ready"),
        ("ERRORGATE_FAMILY_EXPERIMENT_EVALUATED", "errorgate_family_experiment_decision_complete"),
    }
    if (state.get("current_stage"), state.get("current_branch")) not in allowed_states:
        raise ValueError("Error-gate family instantiation is only allowed from ERRORGATE_FAMILY_PREPARED or ERRORGATE_FAMILY_EXPERIMENT_EVALUATED.")

    candidate = _select_next_candidate()
    if candidate is None:
        state["current_stage"] = "ERRORGATE_FAMILY_COMPLETE"
        state["current_branch"] = "errorgate_family_candidate_pool_exhausted"
        state["allowed_next_actions"] = [
            "review the error-gate family results",
            "package the error-gate family evidence",
        ...
        AUTO_SUMMARY_PATH.write_text(
            ...
            "- Active stage: `ERRORGATE_FAMILY_COMPLETE`\n"
            "- Current branch state: `errorgate_family_candidate_pool_exhausted`\n"
            f"- Error-gate family results file: `{RESULTS_CSV.resolve()}`\n",
            encoding="utf-8",
        )
    ...
    state["current_stage"] = "ERRORGATE_FAMILY_EXPERIMENT_READY"
    state["current_branch"] = "errorgate_family_experiment_instance_complete"
    state["active_errorgate_experiment_id"] = candidate["experiment_id"]
    state["active_errorgate_instance_dir"] = str(instance_dir.resolve())
    state["allowed_next_actions"] = [
        "run the instantiated error-gate family experiment",
    ...
    AUTO_SUMMARY_PATH.write_text(
        "# SGDFNet Auto Research Summary\n\n"
        f"- Frozen execution baseline: `{state['frozen_execution_baseline_artifact']}`\n"
        "- Active stage: `ERRORGATE_FAMILY_EXPERIMENT_READY`\n"
        "- Current branch state: `errorgate_family_experiment_instance_complete`\n"
        f"- Active error-gate family experiment: `{candidate['experiment_id']}`\n"
