...
from fusion_family_catalog import list_fusion_family_candidates
...
INSTANCES_DIR = FUSION_DIR / "instances"
RESULTS_CSV = FUSION_DIR / "fusion_family_experiment_results.csv"
BASE_CONFIG = PROJECT_ROOT / "configs" / "v2_fusion_family_base.yaml"
...
def _select_next_candidate() -> dict | None:
    ...
    for candidate in list_fusion_family_candidates():
...
def main() -> None:
    ...
    allowed_states = {
        ("FUSION_FAMILY_PREPARED", "fusion_family_manifest_ready"),
        ("FUSION_FAMILY_EXPERIMENT_EVALUATED", "fusion_family_experiment_decision_complete"),
    }
    if (state.get("current_stage"), state.get("current_branch")) not in allowed_states:
        raise ValueError("Fusion family instantiation is only allowed from the prepared or evaluated family states.")

    candidate = _select_next_candidate()
    if candidate is None:
        state["current_stage"] = "FUSION_FAMILY_COMPLETE"
        state["current_branch"] = "fusion_family_candidate_pool_exhausted"
        state["allowed_next_actions"] = [
            "review the fusion family results",
            "package the fused candidate evidence",
        ...
        AUTO_SUMMARY_PATH.write_text(
            ...
            "- Active stage: `FUSION_FAMILY_COMPLETE`\n"
            "- Current branch state: `fusion_family_candidate_pool_exhausted`\n"
            f"- Fusion family results file: `{RESULTS_CSV.resolve()}`\n",
            encoding="utf-8",
        )
    ...
    state["current_stage"] = "FUSION_FAMILY_EXPERIMENT_READY"
    state["current_branch"] = "fusion_family_experiment_instance_complete"
    state["active_fusion_experiment_id"] = candidate["experiment_id"]
    state["active_fusion_instance_dir"] = str(instance_dir.resolve())
    state["allowed_next_actions"] = [
        "run the instantiated fusion family experiment",
        "compare it against the frozen baseline and accepted modules",
        "decide whether this fusion family should keep expanding",
    ]
    _write_json(STATE_PATH, state)
    ...
    AUTO_SUMMARY_PATH.write_text(
        "# SGDFNet Auto Research Summary\n\n"
        f"- Frozen execution baseline: `{state['frozen_execution_baseline_artifact']}`\n"
        "- Active stage: `FUSION_FAMILY_EXPERIMENT_READY`\n"
        "- Current branch state: `fusion_family_experiment_instance_complete`\n"
        f"- Active fusion experiment: `{candidate['experiment_id']}`\n"
