...
from kickoff_conservative_tailcal_family_catalog import list_kickoff_conservative_tailcal_candidates
...
INSTANCES_DIR = KCTCAL_DIR / "instances"
...
def _select_next_candidate() -> dict | None:
    ...
    for candidate in list_kickoff_conservative_tailcal_candidates():
...
def main() -> None:
    ...
    allowed_states = {
        ("KICKOFF_CONSERVATIVE_TAILCAL_FAMILY_PREPARED", "kickoff_conservative_tailcal_family_manifest_ready"),
        ("KICKOFF_CONSERVATIVE_TAILCAL_EXPERIMENT_EVALUATED", "kickoff_conservative_tailcal_experiment_decision_complete"),
    }
    if (state.get("current_stage"), state.get("current_branch")) not in allowed_states:
        raise ValueError("Kickoff conservative tail-cal instantiation is only allowed from prepared or evaluated states.")

    candidate = _select_next_candidate()
    if candidate is None:
        state["current_stage"] = "KICKOFF_CONSERVATIVE_TAILCAL_FAMILY_COMPLETE"
        state["current_branch"] = "kickoff_conservative_tailcal_family_candidate_pool_exhausted"
        state["allowed_next_actions"] = [
            "review the kickoff conservative tail-calibration family results",
            "package the conservative kickoff tail-calibration evidence",
        ...
        AUTO_SUMMARY_PATH.write_text(
            ...
            "- Active stage: `KICKOFF_CONSERVATIVE_TAILCAL_FAMILY_COMPLETE`\n"
            "- Current branch state: `kickoff_conservative_tailcal_family_candidate_pool_exhausted`\n"
            f"- Kickoff conservative tail-calibration family results file: `{RESULTS_CSV.resolve()}`\n",
            encoding="utf-8",
        )
    ...
    state["current_stage"] = "KICKOFF_CONSERVATIVE_TAILCAL_EXPERIMENT_READY"
    state["current_branch"] = "kickoff_conservative_tailcal_experiment_instance_complete"
    state["active_kickoff_conservative_tailcal_experiment_id"] = candidate["experiment_id"]
    state["active_kickoff_conservative_tailcal_instance_dir"] = str(instance_dir.resolve())
    state["allowed_next_actions"] = [
        "run the instantiated conservative kickoff tail-calibration experiment",
        "compare it against the frozen execution baseline",
        "decide whether this conservative kickoff tail-calibration family should keep expanding",
    ]
    _write_json(STATE_PATH, state)
    ...
    AUTO_SUMMARY_PATH.write_text(
        "# SGDFNet Auto Research Summary\n\n"
        f"- Frozen execution baseline: `{state['frozen_execution_baseline_artifact']}`\n"
        "- Active stage: `KICKOFF_CONSERVATIVE_TAILCAL_EXPERIMENT_READY`\n"
        "- Current branch state: `kickoff_conservative_tailcal_experiment_instance_complete`\n"
        f"- Active conservative kickoff tail-calibration experiment: `{candidate['experiment_id']}`\n"
