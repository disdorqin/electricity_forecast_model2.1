...
from kickoff_followup_cycle_catalog import list_kickoff_followup_candidates
...
INSTANCES_DIR = KFOLLOW_DIR / "instances"
...
def _select_next_candidate() -> dict | None:
    ...
    for candidate in list_kickoff_followup_candidates():
...
def main() -> None:
    ...
    allowed_states = {
        ("KICKOFF_FOLLOWUP_CYCLE_PREPARED", "kickoff_followup_cycle_manifest_ready"),
        ("KICKOFF_FOLLOWUP_EXPERIMENT_EVALUATED", "kickoff_followup_experiment_decision_complete"),
    }
    if (state.get("current_stage"), state.get("current_branch")) not in allowed_states:
        raise ValueError("Kickoff follow-up instantiation is only allowed from kickoff follow-up prepared/evaluated states.")

    candidate = _select_next_candidate()
    if candidate is None:
        state["current_stage"] = "KICKOFF_FOLLOWUP_CYCLE_COMPLETE"
        state["current_branch"] = "kickoff_followup_candidate_pool_exhausted"
        state["allowed_next_actions"] = [
            "review the kickoff follow-up results",
            "package the kickoff follow-up evidence",
            "open another mechanism family if needed",
        ]
        ...
        AUTO_SUMMARY_PATH.write_text(
            ...
            "- Active stage: `KICKOFF_FOLLOWUP_CYCLE_COMPLETE`\n"
            "- Current branch state: `kickoff_followup_candidate_pool_exhausted`\n"
            f"- Kickoff follow-up results file: `{RESULTS_CSV.resolve()}`\n",
            encoding="utf-8",
        )
    ...
    state["current_stage"] = "KICKOFF_FOLLOWUP_EXPERIMENT_READY"
    state["current_branch"] = "kickoff_followup_experiment_instance_complete"
    state["active_kickoff_followup_experiment_id"] = candidate["experiment_id"]
    state["active_kickoff_followup_instance_dir"] = str(instance_dir.resolve())
    state["allowed_next_actions"] = [
        "run the instantiated kickoff follow-up experiment",
        "compare it against the frozen execution baseline",
        "decide whether the kickoff follow-up family shows enough signal to keep expanding",
    ]
    _write_json(STATE_PATH, state)
    ...
    AUTO_SUMMARY_PATH.write_text(
        "# SGDFNet Auto Research Summary\n\n"
        f"- Frozen execution baseline: `{state['frozen_execution_baseline_artifact']}`\n"
        "- Active stage: `KICKOFF_FOLLOWUP_EXPERIMENT_READY`\n"
        "- Current branch state: `kickoff_followup_experiment_instance_complete`\n"
        f"- Active kickoff follow-up experiment: `{candidate['experiment_id']}`\n"
