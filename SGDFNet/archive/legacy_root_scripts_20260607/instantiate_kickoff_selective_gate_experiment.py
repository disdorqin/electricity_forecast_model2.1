...
from kickoff_selective_gate_family_catalog import list_kickoff_selective_gate_candidates
...
INSTANCES_DIR = KSGATE_DIR / "instances"
RESULTS_CSV = KSGATE_DIR / "kickoff_selective_gate_experiment_results.csv"
...
def _select_next_candidate() -> dict | None:
    ...
    for candidate in list_kickoff_selective_gate_candidates():
...
def main() -> None:
    ...
    allowed_states = {
        ("KICKOFF_SELECTIVE_GATE_FAMILY_PREPARED", "kickoff_selective_gate_family_manifest_ready"),
        ("KICKOFF_SELECTIVE_GATE_EXPERIMENT_EVALUATED", "kickoff_selective_gate_experiment_decision_complete"),
    }
    if (state.get("current_stage"), state.get("current_branch")) not in allowed_states:
        raise ValueError("Kickoff selective-gate instantiation is only allowed from prepared or evaluated states.")

    candidate = _select_next_candidate()
    if candidate is None:
        state["current_stage"] = "KICKOFF_SELECTIVE_GATE_FAMILY_COMPLETE"
        state["current_branch"] = "kickoff_selective_gate_family_candidate_pool_exhausted"
        state["allowed_next_actions"] = [
            "review the kickoff selective-gate family results",
            "package the selective-gate evidence",
        ...
        AUTO_SUMMARY_PATH.write_text(
            ...
            "- Active stage: `KICKOFF_SELECTIVE_GATE_FAMILY_COMPLETE`\n"
            "- Current branch state: `kickoff_selective_gate_family_candidate_pool_exhausted`\n"
            f"- Kickoff selective-gate family results file: `{RESULTS_CSV.resolve()}`\n",
            encoding="utf-8",
        )
    ...
    state["current_stage"] = "KICKOFF_SELECTIVE_GATE_EXPERIMENT_READY"
    state["current_branch"] = "kickoff_selective_gate_experiment_instance_complete"
    state["active_kickoff_selective_gate_experiment_id"] = candidate["experiment_id"]
    state["active_kickoff_selective_gate_instance_dir"] = str(instance_dir.resolve())
    state["allowed_next_actions"] = [
        "run the instantiated kickoff selective-gate experiment",
        "compare it against the frozen execution baseline",
        "decide whether this kickoff selective-gate family should keep expanding",
    ]
    _write_json(STATE_PATH, state)
    ...
    AUTO_SUMMARY_PATH.write_text(
        "# SGDFNet Auto Research Summary\n\n"
        f"- Frozen execution baseline: `{state['frozen_execution_baseline_artifact']}`\n"
        "- Active stage: `KICKOFF_SELECTIVE_GATE_EXPERIMENT_READY`\n"
        "- Current branch state: `kickoff_selective_gate_experiment_instance_complete`\n"
        f"- Active kickoff selective-gate experiment: `{candidate['experiment_id']}`\n"
