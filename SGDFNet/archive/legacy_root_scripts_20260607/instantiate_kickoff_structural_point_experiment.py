...
from kickoff_structural_point_family_catalog import list_kickoff_structural_point_candidates
...
INSTANCES_DIR = KSPOINT_DIR / "instances"
RESULTS_CSV = KSPOINT_DIR / "kickoff_structural_point_experiment_results.csv"
BASE_CONFIG = PROJECT_ROOT / "configs" / "protocol_b_a0_j2_val_segment_bias_v1.yaml"
...
def _select_next_candidate() -> dict | None:
    ...
    for candidate in list_kickoff_structural_point_candidates():
...
def main() -> None:
    ...
    allowed_states = {
        ("KICKOFF_STRUCTURAL_POINT_FAMILY_PREPARED", "kickoff_structural_point_family_manifest_ready"),
        ("KICKOFF_STRUCTURAL_POINT_EXPERIMENT_EVALUATED", "kickoff_structural_point_experiment_decision_complete"),
    }
    if (state.get("current_stage"), state.get("current_branch")) not in allowed_states:
        raise ValueError("Kickoff structural-point instantiation is only allowed from prepared or evaluated states.")

    candidate = _select_next_candidate()
    if candidate is None:
        state["current_stage"] = "KICKOFF_STRUCTURAL_POINT_FAMILY_COMPLETE"
        state["current_branch"] = "kickoff_structural_point_family_candidate_pool_exhausted"
        state["allowed_next_actions"] = [
            "review the kickoff structural-point family results",
            "package the structural-point evidence",
        ...
        AUTO_SUMMARY_PATH.write_text(
            ...
            "- Active stage: `KICKOFF_STRUCTURAL_POINT_FAMILY_COMPLETE`\n"
            "- Current branch state: `kickoff_structural_point_family_candidate_pool_exhausted`\n"
            f"- Kickoff structural-point family results file: `{RESULTS_CSV.resolve()}`\n",
            encoding="utf-8",
        )
    ...
    state["current_stage"] = "KICKOFF_STRUCTURAL_POINT_EXPERIMENT_READY"
    state["current_branch"] = "kickoff_structural_point_experiment_instance_complete"
    state["active_kickoff_structural_point_experiment_id"] = candidate["experiment_id"]
    state["active_kickoff_structural_point_instance_dir"] = str(instance_dir.resolve())
    state["allowed_next_actions"] = [
        "run the instantiated kickoff structural-point experiment",
        "compare it against the frozen execution baseline",
        "decide whether this kickoff structural-point family should keep expanding",
    ]
    _write_json(STATE_PATH, state)
    ...
    AUTO_SUMMARY_PATH.write_text(
        "# SGDFNet Auto Research Summary\n\n"
        f"- Frozen execution baseline: `{state['frozen_execution_baseline_artifact']}`\n"
        "- Active stage: `KICKOFF_STRUCTURAL_POINT_EXPERIMENT_READY`\n"
        "- Current branch state: `kickoff_structural_point_experiment_instance_complete`\n"
        f"- Active kickoff structural-point experiment: `{candidate['experiment_id']}`\n"
