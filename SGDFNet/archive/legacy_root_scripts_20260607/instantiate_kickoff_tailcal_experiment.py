...
from kickoff_tailcal_family_catalog import list_kickoff_tailcal_candidates
...
INSTANCES_DIR = KTCAL_DIR / "instances"
RESULTS_CSV = KTCAL_DIR / "kickoff_tailcal_experiment_results.csv"
...
def _select_next_candidate() -> dict | None:
    ...
    for candidate in list_kickoff_tailcal_candidates():
...
def main() -> None:
    ...
    allowed_states = {
        ("KICKOFF_TAILCAL_FAMILY_PREPARED", "kickoff_tailcal_family_manifest_ready"),
        ("KICKOFF_TAILCAL_EXPERIMENT_EVALUATED", "kickoff_tailcal_experiment_decision_complete"),
    }
    if (state.get("current_stage"), state.get("current_branch")) not in allowed_states:
        raise ValueError("Kickoff tail-cal instantiation is only allowed from prepared or evaluated kickoff tail-cal states.")

    candidate = _select_next_candidate()
    if candidate is None:
        state["current_stage"] = "KICKOFF_TAILCAL_FAMILY_COMPLETE"
        state["current_branch"] = "kickoff_tailcal_family_candidate_pool_exhausted"
        state["allowed_next_actions"] = [
            "review the kickoff tail-calibration family results",
            "package the kickoff tail-calibration evidence",
        ...
        AUTO_SUMMARY_PATH.write_text(
            ...
            "- Active stage: `KICKOFF_TAILCAL_FAMILY_COMPLETE`\n"
            "- Current branch state: `kickoff_tailcal_family_candidate_pool_exhausted`\n"
            f"- Kickoff tail-calibration family results file: `{RESULTS_CSV.resolve()}`\n",
            encoding="utf-8",
        )
    ...
    state["current_stage"] = "KICKOFF_TAILCAL_EXPERIMENT_READY"
    state["current_branch"] = "kickoff_tailcal_experiment_instance_complete"
    state["active_kickoff_tailcal_experiment_id"] = candidate["experiment_id"]
    state["active_kickoff_tailcal_instance_dir"] = str(instance_dir.resolve())
    state["allowed_next_actions"] = [
        "run the instantiated kickoff tail-calibration experiment",
        "compare it against the frozen execution baseline",
        "decide whether this kickoff tail-calibration family should keep expanding",
    ]
    _write_json(STATE_PATH, state)
    ...
    AUTO_SUMMARY_PATH.write_text(
        "# SGDFNet Auto Research Summary\n\n"
        f"- Frozen execution baseline: `{state['frozen_execution_baseline_artifact']}`\n"
        "- Active stage: `KICKOFF_TAILCAL_EXPERIMENT_READY`\n"
        "- Current branch state: `kickoff_tailcal_experiment_instance_complete`\n"
        f"- Active kickoff tail-calibration experiment: `{candidate['experiment_id']}`\n"
