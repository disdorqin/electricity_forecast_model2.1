...
from monthhour_family_cycle_catalog import list_monthhour_family_candidates
...
INSTANCES_DIR = MONTHHOUR_DIR / "instances"
RESULTS_CSV = MONTHHOUR_DIR / "monthhour_family_experiment_results.csv"
...
def _select_next_candidate() -> dict | None:
    ...
    for candidate in list_monthhour_family_candidates():
...
def main() -> None:
    ...
    allowed_states = {
        ("MONTHHOUR_FAMILY_PREPARED", "monthhour_family_manifest_ready"),
        ("MONTHHOUR_FAMILY_EXPERIMENT_EVALUATED", "monthhour_family_experiment_decision_complete"),
    }
    if (state.get("current_stage"), state.get("current_branch")) not in allowed_states:
        raise ValueError("Month-hour family instantiation is only allowed from MONTHHOUR_FAMILY_PREPARED or MONTHHOUR_FAMILY_EXPERIMENT_EVALUATED.")

    candidate = _select_next_candidate()
    if candidate is None:
        state["current_stage"] = "MONTHHOUR_FAMILY_COMPLETE"
        state["current_branch"] = "monthhour_family_candidate_pool_exhausted"
        state["allowed_next_actions"] = [
            "review the month-hour family results",
            "package the month-hour family evidence",
        ...
        AUTO_SUMMARY_PATH.write_text(
            ...
            "- Active stage: `MONTHHOUR_FAMILY_COMPLETE`\n"
            "- Current branch state: `monthhour_family_candidate_pool_exhausted`\n"
            f"- Month-hour family results file: `{RESULTS_CSV.resolve()}`\n",
            encoding="utf-8",
        )
    ...
    state["current_stage"] = "MONTHHOUR_FAMILY_EXPERIMENT_READY"
    state["current_branch"] = "monthhour_family_experiment_instance_complete"
    state["active_monthhour_experiment_id"] = candidate["experiment_id"]
    state["active_monthhour_instance_dir"] = str(instance_dir.resolve())
    state["allowed_next_actions"] = [
        "run the instantiated month-hour family experiment",
        "compare it against the frozen execution baseline",
        "decide whether this family should keep expanding",
    ]
    _write_json(STATE_PATH, state)
    ...
    AUTO_SUMMARY_PATH.write_text(
        "# SGDFNet Auto Research Summary\n\n"
        f"- Frozen execution baseline: `{state['frozen_execution_baseline_artifact']}`\n"
        "- Active stage: `MONTHHOUR_FAMILY_EXPERIMENT_READY`\n"
        "- Current branch state: `monthhour_family_experiment_instance_complete`\n"
        f"- Active month-hour family experiment: `{candidate['experiment_id']}`\n"
