...
from next_family_cycle_catalog import list_next_family_cycle_candidates
...
INSTANCES_DIR = NEXT_FAMILY_DIR / "instances"
...
def _select_next_candidate() -> dict | None:
    ...
    for candidate in list_next_family_cycle_candidates():
...
def main() -> None:
    ...
    allowed_states = {
        ("NEXT_FAMILY_CYCLE_PREPARED", "next_family_cycle_manifest_ready"),
        ("NEXT_FAMILY_EXPERIMENT_EVALUATED", "next_family_experiment_decision_complete"),
    }
    if (state.get("current_stage"), state.get("current_branch")) not in allowed_states:
        raise ValueError("Next-family instantiation is only allowed from NEXT_FAMILY_CYCLE_PREPARED or NEXT_FAMILY_EXPERIMENT_EVALUATED.")

    candidate = _select_next_candidate()
    if candidate is None:
        state["current_stage"] = "NEXT_FAMILY_CYCLE_COMPLETE"
        state["current_branch"] = "next_family_candidate_pool_exhausted"
        state["allowed_next_actions"] = [
            "review the next-family experiment results",
            "package the localized-calibration family evidence",
        ...
        AUTO_SUMMARY_PATH.write_text(
            ...
            "- Active stage: `NEXT_FAMILY_CYCLE_COMPLETE`\n"
            "- Current branch state: `next_family_candidate_pool_exhausted`\n"
            f"- Next-family results file: `{RESULTS_CSV.resolve()}`\n",
            encoding="utf-8",
        )
    ...
    state["current_stage"] = "NEXT_FAMILY_EXPERIMENT_READY"
    state["current_branch"] = "next_family_experiment_instance_complete"
    state["active_next_family_experiment_id"] = candidate["experiment_id"]
    state["active_next_family_instance_dir"] = str(instance_dir.resolve())
    state["allowed_next_actions"] = [
        "run the instantiated next-family experiment",
        "compare it against the frozen execution baseline",
        "decide whether this localized-calibration family should keep expanding",
    ]
    _write_json(STATE_PATH, state)
    ...
    AUTO_SUMMARY_PATH.write_text(
        "# SGDFNet Auto Research Summary\n\n"
        f"- Frozen execution baseline: `{state['frozen_execution_baseline_artifact']}`\n"
        "- Active stage: `NEXT_FAMILY_EXPERIMENT_READY`\n"
        "- Current branch state: `next_family_experiment_instance_complete`\n"
        f"- Active next-family experiment: `{candidate['experiment_id']}`\n"
