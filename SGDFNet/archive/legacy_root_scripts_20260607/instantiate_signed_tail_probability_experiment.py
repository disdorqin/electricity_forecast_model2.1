...
from signed_tail_probability_family_catalog import list_signed_tail_probability_candidates
...
INSTANCES_DIR = SIGNED_DIR / "instances"
RESULTS_CSV = SIGNED_DIR / "signed_tail_probability_experiment_results.csv"
BASE_CONFIG = PROJECT_ROOT / "configs" / "v2_signed_tail_probability_base.yaml"
...
def _select_next_candidate() -> dict | None:
    ...
    for candidate in list_signed_tail_probability_candidates():
...
def main() -> None:
    ...
    allowed_states = {
        ("SIGNED_TAIL_PROBABILITY_FAMILY_PREPARED", "signed_tail_probability_family_manifest_ready"),
        ("SIGNED_TAIL_PROBABILITY_EXPERIMENT_EVALUATED", "signed_tail_probability_experiment_decision_complete"),
    }
    if (state.get("current_stage"), state.get("current_branch")) not in allowed_states:
        raise ValueError("Signed-tail probability family instantiation is only allowed from the prepared or evaluated family states.")

    candidate = _select_next_candidate()
    if candidate is None:
        state["current_stage"] = "SIGNED_TAIL_PROBABILITY_FAMILY_COMPLETE"
        state["current_branch"] = "signed_tail_probability_family_candidate_pool_exhausted"
        state["allowed_next_actions"] = [
            "review the signed-tail probability family results",
            "package the signed-tail evidence",
        ...
        AUTO_SUMMARY_PATH.write_text(
            ...
            "- Active stage: `SIGNED_TAIL_PROBABILITY_FAMILY_COMPLETE`\n"
            "- Current branch state: `signed_tail_probability_family_candidate_pool_exhausted`\n"
            f"- Signed-tail probability family results file: `{RESULTS_CSV.resolve()}`\n",
            encoding="utf-8",
        )
    ...
    state["current_stage"] = "SIGNED_TAIL_PROBABILITY_EXPERIMENT_READY"
    state["current_branch"] = "signed_tail_probability_experiment_instance_complete"
    state["active_signed_tail_probability_experiment_id"] = candidate["experiment_id"]
    state["active_signed_tail_probability_instance_dir"] = str(instance_dir.resolve())
    state["allowed_next_actions"] = [
        "run the instantiated signed-tail probability experiment",
        "compare it against the accepted interval module",
        "decide whether this signed-tail family should keep expanding",
    ]
    _write_json(STATE_PATH, state)
    ...
    AUTO_SUMMARY_PATH.write_text(
        "# SGDFNet Auto Research Summary\n\n"
        f"- Frozen execution baseline: `{state['frozen_execution_baseline_artifact']}`\n"
        "- Active stage: `SIGNED_TAIL_PROBABILITY_EXPERIMENT_READY`\n"
        "- Current branch state: `signed_tail_probability_experiment_instance_complete`\n"
        f"- Active signed-tail probability experiment: `{candidate['experiment_id']}`\n"
