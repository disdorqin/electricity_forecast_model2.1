...
from structural_family_cycle_catalog import list_structural_family_candidates
...
INSTANCES_DIR = STRUCTURAL_DIR / "instances"
RESULTS_CSV = STRUCTURAL_DIR / "structural_family_experiment_results.csv"
...
def _select_next_candidate() -> dict | None:
    ...
    for candidate in list_structural_family_candidates():
...
def main() -> None:
    ...
    allowed_states = {
        ("STRUCTURAL_FAMILY_PREPARED", "structural_family_manifest_ready"),
        ("STRUCTURAL_FAMILY_EXPERIMENT_EVALUATED", "structural_family_experiment_decision_complete"),
    }
    if (state.get("current_stage"), state.get("current_branch")) not in allowed_states:
        raise ValueError("Structural family instantiation is only allowed from STRUCTURAL_FAMILY_PREPARED or STRUCTURAL_FAMILY_EXPERIMENT_EVALUATED.")

    candidate = _select_next_candidate()
    if candidate is None:
        state["current_stage"] = "STRUCTURAL_FAMILY_COMPLETE"
        state["current_branch"] = "structural_family_candidate_pool_exhausted"
        state["allowed_next_actions"] = [
            "review the structural family results",
            "package the structural family evidence",
            "open another new mechanism family for the next automated cycle",
        ]
        ...
        AUTO_SUMMARY_PATH.write_text(
            ...
            "- Active stage: `STRUCTURAL_FAMILY_COMPLETE`\n"
            "- Current branch state: `structural_family_candidate_pool_exhausted`\n"
            f"- Structural family results file: `{RESULTS_CSV.resolve()}`\n",
            encoding="utf-8",
        )
    ...
    state["current_stage"] = "STRUCTURAL_FAMILY_EXPERIMENT_READY"
    state["current_branch"] = "structural_family_experiment_instance_complete"
    state["active_structural_experiment_id"] = candidate["experiment_id"]
    state["active_structural_instance_dir"] = str(instance_dir.resolve())
    state["allowed_next_actions"] = [
        "run the instantiated structural family experiment",
        "compare it against the frozen execution baseline",
        "decide whether this main-model family should keep expanding",
    ]
    _write_json(STATE_PATH, state)
    ...
    AUTO_SUMMARY_PATH.write_text(
        "# SGDFNet Auto Research Summary\n\n"
        f"- Frozen execution baseline: `{state['frozen_execution_baseline_artifact']}`\n"
        "- Active stage: `STRUCTURAL_FAMILY_EXPERIMENT_READY`\n"
        "- Current branch state: `structural_family_experiment_instance_complete`\n"
        f"- Active structural family experiment: `{candidate['experiment_id']}`\n"
