...
def main() -> None:
    ...
    if state.get("current_stage") != "ERRORGATE_FAMILY_EXPERIMENT_READY" or state.get("current_branch") != "errorgate_family_experiment_instance_complete":
        raise ValueError("Error-gate family experiment execution is only allowed from ERRORGATE_FAMILY_EXPERIMENT_READY.")
    ...
    state["current_stage"] = "ERRORGATE_FAMILY_EXPERIMENT_RAN"
    state["current_branch"] = "errorgate_family_experiment_run_complete"
    state["last_candidate_artifact"] = run_dir
    state["allowed_next_actions"] = [
        "evaluate the error-gate family experiment",
    ...
    AUTO_SUMMARY_PATH.write_text(
        ...
        "- Active stage: `ERRORGATE_FAMILY_EXPERIMENT_RAN`\n"
        "- Current branch state: `errorgate_family_experiment_run_complete`\n"
