...
def main() -> None:
    ...
    if state.get("current_stage") != "FOLLOWUP_EXPERIMENT_READY" or state.get("current_branch") != "followup_experiment_instance_complete":
        raise ValueError("Follow-up experiment execution is only allowed from FOLLOWUP_EXPERIMENT_READY.")
    ...
    state["current_stage"] = "FOLLOWUP_EXPERIMENT_RAN"
    state["current_branch"] = "followup_experiment_run_complete"
    state["last_candidate_artifact"] = run_dir
    state["allowed_next_actions"] = [
        "evaluate the follow-up experiment",
    ...
    AUTO_SUMMARY_PATH.write_text(
        ...
        "- Active stage: `FOLLOWUP_EXPERIMENT_RAN`\n"
        "- Current branch state: `followup_experiment_run_complete`\n"
