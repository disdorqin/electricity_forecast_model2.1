...
def main() -> None:
    ...
    if state.get("current_stage") != "FUSION_FAMILY_EXPERIMENT_READY" or state.get("current_branch") != "fusion_family_experiment_instance_complete":
        raise ValueError("Fusion family experiment run is only allowed from FUSION_FAMILY_EXPERIMENT_READY.")
    ...
    cmd = [sys.executable, str(PROJECT_ROOT / "run_fusion_release_bundle.py"), "--config", str(config_path)]
    ...
    state["current_stage"] = "FUSION_FAMILY_EXPERIMENT_RAN"
    state["current_branch"] = "fusion_family_experiment_run_complete"
    state["last_candidate_artifact"] = manifest["run_artifact"]
    state["allowed_next_actions"] = [
        "evaluate the fusion family experiment",
    ...
    AUTO_SUMMARY_PATH.write_text(
        ...
        "- Active stage: `FUSION_FAMILY_EXPERIMENT_RAN`\n"
        "- Current branch state: `fusion_family_experiment_run_complete`\n"
