...
def main() -> None:
    ...
    if state.get("current_stage") != "KICKOFF_FOLLOWUP_EXPERIMENT_READY" or state.get("current_branch") != "kickoff_followup_experiment_instance_complete":
        raise ValueError("Kickoff follow-up execution is only allowed from KICKOFF_FOLLOWUP_EXPERIMENT_READY.")
    ...
    state["current_stage"] = "KICKOFF_FOLLOWUP_EXPERIMENT_RAN"
    state["current_branch"] = "kickoff_followup_experiment_run_complete"
    state["last_candidate_artifact"] = run_dir
    state["allowed_next_actions"] = [
        "evaluate the kickoff follow-up experiment",
        "compare the kickoff follow-up family against the frozen baseline",
        "either retain the family or stop with a negative result package",
    ]
    ...
    AUTO_SUMMARY_PATH.write_text(
        ...
        "- Active stage: `KICKOFF_FOLLOWUP_EXPERIMENT_RAN`\n"
        "- Current branch state: `kickoff_followup_experiment_run_complete`\n"
