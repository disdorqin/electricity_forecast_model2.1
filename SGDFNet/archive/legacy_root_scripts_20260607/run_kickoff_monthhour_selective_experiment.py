...
def main() -> None:
    ...
    if state.get("current_stage") != "KICKOFF_MONTHHOUR_SELECTIVE_EXPERIMENT_READY" or state.get("current_branch") != "kickoff_monthhour_selective_experiment_instance_complete":
        raise ValueError("Kickoff month-hour selective run is only allowed from KICKOFF_MONTHHOUR_SELECTIVE_EXPERIMENT_READY.")
    ...
    cmd = [sys.executable, str(PROJECT_ROOT / "run_monthhour_selective_calibration.py"), "--config", str(config_path)]
    ...
    state["current_stage"] = "KICKOFF_MONTHHOUR_SELECTIVE_EXPERIMENT_RAN"
    state["current_branch"] = "kickoff_monthhour_selective_experiment_run_complete"
    state["last_candidate_artifact"] = manifest["run_artifact"]
    state["allowed_next_actions"] = [
        "evaluate the kickoff month-hour selective experiment",
    ...
    AUTO_SUMMARY_PATH.write_text(
        ...
        "- Active stage: `KICKOFF_MONTHHOUR_SELECTIVE_EXPERIMENT_RAN`\n"
        "- Current branch state: `kickoff_monthhour_selective_experiment_run_complete`\n"
