...
def main() -> None:
    ...
    if state.get("current_stage") != "KICKOFF_STRUCTURAL_POINT_EXPERIMENT_READY" or state.get("current_branch") != "kickoff_structural_point_experiment_instance_complete":
        raise ValueError("Kickoff structural-point run is only allowed from KICKOFF_STRUCTURAL_POINT_EXPERIMENT_READY.")
    ...
    state["current_stage"] = "KICKOFF_STRUCTURAL_POINT_EXPERIMENT_RAN"
    state["current_branch"] = "kickoff_structural_point_experiment_run_complete"
    state["last_candidate_artifact"] = manifest["run_artifact"]
    state["allowed_next_actions"] = [
        "evaluate the kickoff structural-point experiment",
    ...
    AUTO_SUMMARY_PATH.write_text(
        ...
        "- Active stage: `KICKOFF_STRUCTURAL_POINT_EXPERIMENT_RAN`\n"
        "- Current branch state: `kickoff_structural_point_experiment_run_complete`\n"
