...
def main() -> None:
    ...
    if state.get("current_stage") != "MONTHHOUR_FAMILY_EXPERIMENT_READY" or state.get("current_branch") != "monthhour_family_experiment_instance_complete":
        raise ValueError("Month-hour family experiment execution is only allowed from MONTHHOUR_FAMILY_EXPERIMENT_READY.")
    ...
    state["current_stage"] = "MONTHHOUR_FAMILY_EXPERIMENT_RAN"
    state["current_branch"] = "monthhour_family_experiment_run_complete"
    state["last_candidate_artifact"] = run_dir
    state["allowed_next_actions"] = [
        "evaluate the month-hour family experiment",
        "compare the new family against the frozen baseline",
    ...
    AUTO_SUMMARY_PATH.write_text(
        ...
        "- Active stage: `MONTHHOUR_FAMILY_EXPERIMENT_RAN`\n"
        "- Current branch state: `monthhour_family_experiment_run_complete`\n"
