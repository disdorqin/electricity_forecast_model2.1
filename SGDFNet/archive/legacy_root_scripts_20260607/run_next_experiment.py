...
def main() -> None:
    ...
    if state.get("current_stage") != "FRESH_CYCLE_EXPERIMENT_READY" or state.get("current_branch") != "fresh_cycle_experiment_instance_complete":
        raise ValueError("Fresh-cycle experiment execution is only allowed from FRESH_CYCLE_EXPERIMENT_READY.")
    ...
    state["current_stage"] = "FRESH_CYCLE_EXPERIMENT_RAN"
    state["current_branch"] = "fresh_cycle_experiment_run_complete"
    state["last_candidate_artifact"] = run_dir
    state["allowed_next_actions"] = [
        "evaluate the executed fresh-cycle experiment",
        "compare the result against the frozen baseline with consistent metrics",
        "decide whether to keep, reject, switch, or stop",
    ]
    ...
    AUTO_SUMMARY_PATH.write_text(
        ...
        "- Active stage: `FRESH_CYCLE_EXPERIMENT_RAN`\n"
        "- Current branch state: `fresh_cycle_experiment_run_complete`\n"
