...
def main() -> None:
    ...
    if state.get("current_stage") != "NEXT_FAMILY_EXPERIMENT_READY" or state.get("current_branch") != "next_family_experiment_instance_complete":
        raise ValueError("Next-family experiment execution is only allowed from NEXT_FAMILY_EXPERIMENT_READY.")
    ...
    state["current_stage"] = "NEXT_FAMILY_EXPERIMENT_RAN"
    state["current_branch"] = "next_family_experiment_run_complete"
    state["last_candidate_artifact"] = run_dir
    state["allowed_next_actions"] = [
        "evaluate the next-family experiment",
        "compare the localized-calibration family against the frozen baseline",
    ...
    AUTO_SUMMARY_PATH.write_text(
        ...
        "- Active stage: `NEXT_FAMILY_EXPERIMENT_RAN`\n"
        "- Current branch state: `next_family_experiment_run_complete`\n"
