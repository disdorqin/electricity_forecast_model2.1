...
def main() -> None:
    ...
    if state.get("current_stage") != "SIGNED_TAIL_PROBABILITY_EXPERIMENT_READY" or state.get("current_branch") != "signed_tail_probability_experiment_instance_complete":
        raise ValueError("Signed-tail probability experiment run is only allowed from SIGNED_TAIL_PROBABILITY_EXPERIMENT_READY.")
    ...
    cmd = [sys.executable, str(PROJECT_ROOT / "run_signed_tail_probability.py"), "--config", str(config_path)]
    ...
    state["current_stage"] = "SIGNED_TAIL_PROBABILITY_EXPERIMENT_RAN"
    state["current_branch"] = "signed_tail_probability_experiment_run_complete"
    state["last_candidate_artifact"] = manifest["run_artifact"]
    state["allowed_next_actions"] = [
        "evaluate the signed-tail probability experiment",
    ...
    AUTO_SUMMARY_PATH.write_text(
        ...
        "- Active stage: `SIGNED_TAIL_PROBABILITY_EXPERIMENT_RAN`\n"
        "- Current branch state: `signed_tail_probability_experiment_run_complete`\n"
