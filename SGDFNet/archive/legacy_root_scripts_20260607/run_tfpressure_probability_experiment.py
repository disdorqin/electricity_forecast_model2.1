...
def main() -> None:
    ...
    if state.get("current_stage") != "TFPRESSURE_PROBABILITY_EXPERIMENT_READY" or state.get("current_branch") != "tfpressure_probability_experiment_instance_complete":
        raise ValueError("TF+pressure probability experiment run is only allowed from TFPRESSURE_PROBABILITY_EXPERIMENT_READY.")
    ...
    cmd = [sys.executable, str(PROJECT_ROOT / "run_v2_probability.py"), "--config", str(config_path)]
    ...
    state["current_stage"] = "TFPRESSURE_PROBABILITY_EXPERIMENT_RAN"
    state["current_branch"] = "tfpressure_probability_experiment_run_complete"
    state["last_candidate_artifact"] = manifest["run_artifact"]
    state["allowed_next_actions"] = [
        "evaluate the TF+pressure probability experiment",
        "compare it against the frozen execution baseline and accepted interval module",
    ...
    AUTO_SUMMARY_PATH.write_text(
        ...
        "- Active stage: `TFPRESSURE_PROBABILITY_EXPERIMENT_RAN`\n"
        "- Current branch state: `tfpressure_probability_experiment_run_complete`\n"
